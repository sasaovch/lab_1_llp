...
void save_meta_page_and_create_new(Cursor* cursor) {
    ...
    cursor->page = page;
    printf("Save and create new page\n");
}
...
void save_type(Cursor* cursor, MetaTable* meta_page) {
    ...
    if (cursor->page->page_header->offset + META_TABLE_SIZE > PAGE_SIZE) {
        printf("Need to create new page\n");
...
void read_all_nodes(Cursor* cursor) {
    ...
    do {
        ...
        for (int i = 0; i < (pageHeader->offset / META_TABLE_SIZE); i++) {
            ...
            print_type(metaPage);
            read_nodes_by_type(cursor, metaPage);
        }
        page_num++;
    } while (page_num * PAGE_SIZE < file_length);
}
...
void insert_relationship(Cursor* cursor) {
    Relationship relationships[] = {
        ...
        {"brother", 8, "sasaovch", 6, "andrey"},
        {"brother", 6, "andrey", 8, "sasaovch"},
    };
}
...
// void select_types(Cursor* cursor) {
//     cursor->offset = 0;
//     MetaTable* meta_page = malloc(META_TABLE_SIZE);
//     read_all_types(cursor, meta_page, NODE);
//     print_type(meta_page);
//     free(meta_page);

//     meta_page = malloc(META_TABLE_SIZE);
//     read_types(cursor, meta_page, RELATIONSHIP);
//     print_type(meta_page);
//     free(meta_page);

//     meta_page = malloc(META_TABLE_SIZE);
//     read_types(cursor, meta_page, PROPERTY);
//     print_type(meta_page);
//     free(meta_page);
// }
