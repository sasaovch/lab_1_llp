...
uint32_t zero = ZERO;
        ...
        last_block = first_header->block_number;
    }

    set_pointer_offset_file(cursor->file, first_header->block_number * PAGE_SIZE + PAGE_HEADER_SIZE);
    write_to_file(cursor->file, empty_block, PAGE_BODY_SIZE);    
...
uint32_t find_block_before(Cursor* cursor, const PageHeader* page_header, Entity* entity) {
